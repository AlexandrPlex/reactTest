       ...
       <form>
           {
               Object.keys(this.props.headerNewItem).map((key: string) => {
                   return(<div key={key}> <label>{this.props.headerNewItem[key]} </label> <input onChange={this.onChengeInput.bind(this, key)} type="email" className="form-control"/></div> )
               })
           }
           <br />
           ...
           <button type="button" className="btn btn-success" onClick={this.props.onHandleAddNewItem.bind(this, this.state.addValue)}>Добавить</button>
       </form>
    );
  }
}
